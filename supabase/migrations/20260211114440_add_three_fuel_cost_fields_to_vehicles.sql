/*
  ...
  1. Changes to vehicles table
    - Remove the single `standard_fuel_cost_per_km` field
...
BEGIN
  ...
  -- Remove the old single field if it exists
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'vehicles' AND column_name = 'standard_fuel_cost_per_km'
  ) THEN
    -- Optionally copy data to one of the new fields before dropping
    UPDATE vehicles SET standard_fuel_cost_dry = standard_fuel_cost_per_km WHERE standard_fuel_cost_dry = 0;
    ALTER TABLE vehicles DROP COLUMN standard_fuel_cost_per_km;
  END IF;
END $$;
